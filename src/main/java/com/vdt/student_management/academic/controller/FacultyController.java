...
public class FacultyController {
    ...
    ResponseEntity<ApiResponse<FacultyDetailResponse>> getFacultyById(@PathVariable("id") Long id) {
        return ResponseEntity.ok(ApiResponse.<FacultyDetailResponse>builder()
                .code(200)
                .data(facultyService.getFacultyById(id))
                .build());
    }

    @PostMapping
    @PreAuthorize("hasRole(T(com.vdt.student_management.common.enums.RoleType).ADMIN)")
    ResponseEntity<ApiResponse<FacultyDetailResponse>> addFaculty(@RequestBody AddFacultyRequest request) {

        return ResponseEntity.ok(ApiResponse.<FacultyDetailResponse>builder()
                .code(201)
                .data(facultyService.upsertFaculty(null, request))
                .build());
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole(T(com.vdt.student_management.common.enums.RoleType).ADMIN)")
    ResponseEntity<ApiResponse<FacultyDetailResponse>> updateFaculty(
            @PathVariable("id") Long id, @RequestBody AddFacultyRequest request) {
        return ResponseEntity.ok(ApiResponse.<FacultyDetailResponse>builder()
                .code(200)
                .data(facultyService.upsertFaculty(id, request))
                .build());
    }
