...
public class StudentClassController {
    ...
    @GetMapping
    ResponseEntity<ApiResponse<PageResponse<StudentClassResponse>>> getAllStudentClasses(Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.<PageResponse<StudentClassResponse>>builder()
                .code(200)
                .data(PageResponse.fromPage(studentClassService.getAllStudentClasses(pageable)))
                .build());
    }

    @GetMapping("/{id}")
    ResponseEntity<ApiResponse<StudentClassResponse>> getStudentClassById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.<StudentClassResponse>builder()
                .code(200)
                .data(studentClassService.getStudentClassById(id))
                .build());
    }

    @PostMapping
    @PreAuthorize("hasRole(T(com.vdt.student_management.common.enums.RoleType).ADMIN)")
    ResponseEntity<ApiResponse<StudentClassResponse>> addStudentClass(@RequestBody AddStudentClassRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.<StudentClassResponse>builder()
                        .code(201)
                        .data(studentClassService.upsertStudentClass(null, request))
                        .build());
    }
    ...
    ResponseEntity<ApiResponse<StudentClassResponse>> updateStudentClass(
        ...
        return ResponseEntity.ok(ApiResponse.<StudentClassResponse>builder()
                .code(200)
                .data(studentClassService.upsertStudentClass(id, request))
                .build());
    }
