...
public class StudentController {
    ...
    ResponseEntity<ApiResponse<StudentResponse>> addStudent(@RequestBody AddStudentRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.<StudentResponse>builder()
                        .code(201)
                        .data(studentService.upsertStudent(null, request))
                        .build());
    }
    ...
    ResponseEntity<ApiResponse<StudentResponse>> getStudentById(@PathVariable("id") Long id) {
        ...
        var response = ApiResponse.<StudentResponse>builder()
                .code(200)
    ...
    ResponseEntity<ApiResponse<PageResponse<StudentResponse>>> getStudents(Pageable pageable) {
        var response = ApiResponse.<PageResponse<StudentResponse>>builder()
                .code(200)
                .data(PageResponse.fromPage(studentService.getStudents(pageable)))
                .build();
    ...
    @PutMapping("/{id}")
    ResponseEntity<ApiResponse<StudentResponse>> updateStudent(
            @PathVariable("id") Long id, @RequestBody AddStudentRequest request) {

        var response = ApiResponse.<StudentResponse>builder()
                .code(200)
                .data(studentService.upsertStudent(id, request))
                .build();
        return ResponseEntity.ok(response);
    }
